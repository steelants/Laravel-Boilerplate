...
var Editor = /*#__PURE__*/function () {
  function Editor(context) {
    ...
    this.createLink = this.wrapCommand(function (linkInfo) {
      ...
      if (isTextChanged) {
        rng = rng.deleteContents();
        var anchor = rng.insertNode(external_jQuery_default()('<A></A>').text(linkText)[0]);
        anchors.push(anchor);
      } else {
      ...
      external_jQuery_default().each(anchors, function (idx, anchor) {
        ...
        if (isNewWindow) {
          ...
          if (rel.length) {
            external_jQuery_default()(anchor).attr('rel', rel.join(' '));
          }
        } else {
  ...
  HintPopover_createClass(HintPopover, [{
    ...
    value: function initialize() {
      ...
      this.$content.on('click', '.note-hint-item', function (event) {
        ...
        external_jQuery_default()(event.currentTarget).addClass('active');
...
external_jQuery_default()(document).on('click.note-dropdown-menu', function (e) {
  if (!external_jQuery_default()(e.target).closest('.note-btn-group').length) {
    external_jQuery_default()('.note-btn-group.open .note-btn.active').removeClass('active');
