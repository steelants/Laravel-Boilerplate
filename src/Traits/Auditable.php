...
trait Auditable
{
	...
	protected static function createdBy($model)
    {
		...
		$activity->lang_text = __('Created :model', ['model' => __(class_basename($model) . " " . $model->{self::$nameColumn})]);
    ...
    {
		$activity = new Activity();
        $activity->lang_text = __('Removed :model', ['model' => __(class_basename($model) . " " . $model->{self::$nameColumn})]);
        $activity->save();
	}
}
